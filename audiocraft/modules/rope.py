...
class XPos(nn.Cell):
    ...
    def __init__(self, dim: int, smoothing: float = 0.4, base_scale: int = 512,
                 device=None, dtype: mindspore.dtype = mindspore.float32):
        super().__init__()
        assert dim % 2 == 0
        assert dtype in [mindspore.float64, mindspore.float32]
        self.dtype = dtype
        self.base_scale = base_scale

        half_dim = dim // 2
        adim = ops.arange(half_dim, device=device, dtype=dtype)
        decay_rates = (adim / half_dim + smoothing) / (1.0 + smoothing)
        self.register_buffer("decay_rates", decay_rates)
        self.decay: tp.Optional[Tensor] = None
    ...
    def get_decay(self, start: int, end: int):
        ...
        if self.decay is None or end > self.decay.shape[0]:
            assert isinstance(self.decay_rates, Tensor)  # Satisfy type checker.
            idx = ops.arange(end, device=self.decay_rates.device, dtype=self.dtype)
            power = idx / self.base_scale
            scale = self.decay_rates ** power.unsqueeze(-1)
            self.decay = ops.polar(scale, ops.zeros_like(scale))
...
class RotaryEmbedding(nn.Cell):
    ...
    def __init__(self, dim: int, max_period: float = 10000.0, xpos: bool = False,
                 scale: float = 1.0, device=None, dtype: mindspore.dtype = mindspore.float32):
        super().__init__()
        assert dim % 2 == 0
        self.scale = scale
        assert dtype in [mindspore.float64, mindspore.float32]
        ...
        adim = ops.arange(0, dim, 2, device=device, dtype=dtype)[: (dim // 2)]
        frequencies = 1.0 / (max_period ** (adim / dim))
        self.register_buffer("frequencies", frequencies)
        self.rotation: tp.Optional[Tensor] = None
    ...
    def get_rotation(self, start: int, end: int):
        ...
        if self.rotation is None or end > self.rotation.shape[0]:
            assert isinstance(self.frequencies, Tensor)  # Satisfy type checker.
            idx = ops.arange(end, device=self.frequencies.device, dtype=self.dtype)
            angles = ops.outer(idx, self.frequencies)
            self.rotation = ops.polar(ops.ones_like(angles), angles)
        return self.rotation[start:end]

    def rotate(self, x: Tensor, start: int = 0, invert_decay: bool = False):
        ...
        x_complex = ops.view_as_complex(x.to(self.dtype).reshape(*x.shape[:-1], -1, 2))
        scaled_rotation = (rotation * decay) * self.scale + (1.0 - self.scale)
        x_out = ops.view_as_real(x_complex * scaled_rotation).flatten(-2)

        return x_out.type_as(x)

    def rotate_qk(self, query: Tensor, key: Tensor, start: int = 0):
